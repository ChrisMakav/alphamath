...
interface CoursesContentProps {
  /**
   * Niveaux visibles pour l'utilisateur connecté : "all" (admin, ou enseignant non premium n'ayant
   * pas de restriction explicite — cf. getAccessibleLevels), ou la liste exacte des niveaux
   * autorisés (un seul niveau pour élève/parent/enseignant non premium, toute une catégorie pour
   * un enseignant premium). "all" aussi pour les visiteurs anonymes.
   */
  accessibleLevels: SchoolLevel[] | "all";
}

function CoursesContent({ accessibleLevels }: CoursesContentProps) {
  ...
  const isRestricted = accessibleLevels !== "all";
  const singleLevel = isRestricted && accessibleLevels.length === 1 ? accessibleLevels[0] : null;

  const [search, setSearch] = useState("");
  const [level, setLevel] = useState<string>(singleLevel ?? "all");
  const [subject, setSubject] = useState("all");

  const availableCourses = useMemo(
    () => (isRestricted ? COURSES.filter((c) => accessibleLevels.includes(c.schoolLevel)) : COURSES),
    [isRestricted, accessibleLevels]
  );

  const levelOptions = isRestricted
    ? LEVEL_OPTIONS.filter((opt) => opt.id === "all" || accessibleLevels.includes(opt.id as SchoolLevel))
    : LEVEL_OPTIONS;
  ...
  }, [availableCourses, search, level, subject, category]);
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-12">
      ...
      {isRestricted && (
        <div
          className="flex items-center gap-4 mb-8 px-5 py-4 rounded-[var(--am-radius-xl)] flex-wrap"
          style={{ background: "var(--am-purple-muted)", border: "1px solid var(--am-purple)" }}
        >
          <p className="text-sm text-[var(--am-text)]">
            {singleLevel ? (
              <>
                Tu vois les cours de niveau <strong>{LEVEL_LABELS[singleLevel]}</strong>. L&apos;accès à
                plusieurs niveaux à la fois est réservé aux administrateurs, et aux enseignants premium
                pour leur propre catégorie (collège, lycée ou supérieur).
              </>
            ) : (
              <>
                Tu vois les cours de ta catégorie. L&apos;accès à toutes les catégories (collège, lycée,
                supérieur) à la fois est réservé aux administrateurs.
              </>
            )}
          </p>
        </div>
...
export function CourseExplorer({ accessibleLevels }: CoursesContentProps) {
  return (
    <Suspense fallback={null}>
      <CoursesContent accessibleLevels={accessibleLevels} />
    </Suspense>
  );
}
